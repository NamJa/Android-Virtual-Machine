...
binary 후보: `tools/create_debug_guest_fixture.sh` 가 생성한 minimal rootfs 안의 `system/bin/sh` 또는 `system/bin/toybox`.
...
- arm64 PIE binary `/system/bin/sh -c 'echo hello'` 가 guest 안에서 실행.
...
| `tools/create_debug_guest_fixture.sh` | minimal `sh` 또는 `toybox` 포함 |
...
1. `runGuestBinary("vm1", "/system/bin/sh", arrayOf("-c", "echo hello"), 5000)` 실행.
2. exit code == 0 인지 확인.
...
```text
STAGE_PHASE_B_BINARY passed=true binary=/system/bin/sh exit=0 stdout=hello
```
...
- guest binary 가 의존하는 라이브러리 (`/system/lib64/libc.so`, `/system/lib64/libdl.so`) 가 fixture 에 포함되어야 함. fixture 빌드 스크립트 갱신 필수.
