...
- `BIND_VPN_SERVICE` 는 runtime permission 이 아니므로 manifest 의 `VpnService` 선언에만 사용하고, 실제 활성화 시점에는 `VpnService.prepare()` 로 사용자 동의를 받는다.
- DNS proxy 옵션 추가.
...
| `app/src/main/AndroidManifest.xml` | `<service android:permission="android.permission.BIND_VPN_SERVICE">` 선언 |
| `app/src/main/cpp/syscall/socket.cpp` | mode 별 라우팅 |

### 5.7.4 세부 작업

#### D.7.a 사용자 동의와 VpnService.Builder

- bridge mode 를 `vpn_isolated` 로 바꿀 때 host 는 `VpnService.prepare(context)` 를 먼저 호출한다.
- prepare intent 가 반환되면 사용자에게 Android 시스템 VPN 동의 화면을 보여준다.
- 동의 전에는 VPN fd 를 만들지 않고, guest network 는 기존 policy 결과(`UNAVAILABLE` 또는 `host_nat`) 로 유지한다.
- 동의 후에만 아래 builder 로 tun fd 를 생성한다.
...
- 진단 라인: `STAGE_PHASE_D_NETWORK_ISOLATION passed=true vpn_consent=granted vpn_attached=true egress_mode=vpn_isolated dns_proxy=on`.

### 5.7.6 위험

- VpnService 는 사용자에게 시스템 동의 다이얼로그가 떠야 함. 첫 enable 시 명확한 reason UI 필요.
