...
5. §7 VmInstanceService 통합 — 🟡 착수: boot-mode 게이트(`GuestBootPolicy`, bootReady+flag) 배선·on-device 실증(`guest boot mode=SIMULATED bootReady=false`, flag off). REAL 경로(spike→production 승격 + native 실부팅 + flag on)는 ROM 확보 후 — 설계는 `vm-boot-integration-design.md`. 이 flag를 켜는 것이 G1 `passed=true`로 가는 마지막 스위치.
