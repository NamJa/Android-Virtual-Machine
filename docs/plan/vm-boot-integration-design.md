...
> 목적: EP2에서 에뮬레이터로 실증한 Option B 메커니즘(부트스트랩·seccomp 게이트웨이·openat VFS 서비싱)을 **제품 부팅 경로(`VmInstanceService.startRuntime`)에 통합**해, 실제 게스트 ROM이 부팅하도록 만드는 마지막 단계(G1 `passed=true`)의 설계. 이 문서는 설계 + 착수 증분(boot-mode 게이트)이며, 실제 부팅 실증은 클린룸 ROM 확보 후.
...
```
[착수, 이번 턴] GuestBootPolicy(게이트) + startRuntime 배선/로깅 + 본 설계   ✅
        │
        ▼
...
- 이번 턴: `GuestBootPolicyTest`(JVM 4/4) — 게이트 매트릭스. `startRuntime` 배선은 컴파일 + on-device 로그로 관측(현재 SIMULATED).
...
## 7. 이번 턴 변경 파일
- `vm/GuestBootPolicy.kt` (+ `GuestBootPolicyTest`)
- `vm/VmInstanceService.kt` (boot-mode 계산·로깅 배선)
- 본 문서
